...
var checkUserLoggedIn = function() {
	window.user_logged_in = $('#logged-in-user');
}
...
$(document).ready(function() {
	...
	if ( window.user_logged_in.length > 0 ) {
	  window.pollInterval = window.setInterval(pollActivity, 5000);
	  pollActivity();
	}
	else {
		if (window.pollInterval && window.pollInterval > 0) {
			console.log("About to clear pollinterval:" + window.pollInterval);
			window.clearInterval(window.pollInterval);
		}
	}
});